...
def process_video(face_detector, video_p, crop_dir, args):
    if args.fake_type == "real":
        maks_p = video_p.replace("original_sequences/youtube/{}".format(args.quality),
                                 "manipulated_sequences/Deepfakes/masks")
    ...
    space = frames_num // args.max_num
    frames_num = min(frames_num, space * args.max_num)
...
def main(args):
    with open(os.path.join(args.src_root, "split/te.json")) as f:
        pairs = json.load(f)
    ...
    for a, b in pairs:
        if args.fake_type == "real":
            video_ps.append(
                osp.join(args.src_root, 'original_sequences/youtube/{}/videos/{}.mp4'.format(args.quality, a)))
            video_ps.append(
                osp.join(args.src_root, 'original_sequences/youtube/{}/videos/{}.mp4'.format(args.quality, b)))
        else:
            video_ps.append(osp.join(args.src_root,
                                     'manipulated_sequences/{}/{}/videos/{}_{}.mp4'.format(args.fake_type, args.quality,
                                                                                           a, b)))
            video_ps.append(osp.join(args.src_root,
                                     'manipulated_sequences/{}/{}/videos/{}_{}.mp4'.format(args.fake_type, args.quality,
                                                                                           b, a)))
...
if __name__ == '__main__':
    ...
    arg('--fake-type', type=str, default='Deepfakes',
        choices=["Deepfakes", "Face2Face", "FaceSwap", "NeuralTextures", "DeepFakeDetection", "real"])
    ...
    arg('--max-num', type=int, default=20)
